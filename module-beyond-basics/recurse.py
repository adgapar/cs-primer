...
def palindrome(letters):
    ...
    letters = "".join(char.lower() for char in letters if char.isalpha())
...
def filter_recursive(f, xs):
    ...
    elem = xs[0]
    if f(elem):
        return [elem] + filter_recursive(f, xs[1:])
    else:
...
def reduce(f, xs, init=None):
    ...
    xs = iter(xs)
    new_init = next(xs) if init is None else init
    x = next(xs)
    new_init = f(new_init, x)

    return reduce(f, list(xs), new_init)
